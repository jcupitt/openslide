...
static const char ***allowed_level_type_strings = {
     { "ORIGINAL", "PRIMARY", "VOLUME", "NONE" },
     { "DERIVED", "PRIMARY", "VOLUME", "RESAMPLED" },
};

static const struct allowed_level_types = {
    allowed_level_type_strings, 
    sizeof(allowed_level_type_strings) / sizeof(allowed_level_type_strings[0])
};

// the ImageTypes we allow for associated images
static const char ***allowed_associated_type_strings = {
    { "ORIGINAL", "PRIMARY", "LABEL", "NONE" },
    { "ORIGINAL", "PRIMARY", "OVERVIEW", "NONE" },
};

static const struct allowed_associated_types = {
    allowed_associated_type_strings, 
    sizeof(allowed_associated_type_strings) / 
        sizeof(allowed_associated_type_strings[0])
};

G_DEFINE_AUTOPTR_CLEANUP_FUNC(DcmFilehandle, dcm_dicom_destroy)
...
static void dicom_file_destroy(struct dicom_file *d) {
  FREEF(dcm_filehandle_destroy, d->filehandle);
  FREEF(dcm_dataset_destroy, d->file_metadata);
  FREEF(dcm_dataset_destroy, d->metadata);
  FREEF(dcm_bot_destroy, d->bot);
  g_free(d->filename);
  g_slice_free(struct dicom_file, d);
}

static bool
get_tag_int(DcmDataSet *dataset, const char *keyword, int *result)
{
    uint32_t tag = dcm_dict_tag_from_keyword(keyword);
    DcmElement *element = dcm_dataset_get(NULL, dataset, tag);
    int64_t value;
    if (!dcm_element_get_value_integer(NULL, element, &value, 0)) {
        return false;
    }
    *result = value;
    return true;
...
static bool
get_tag_str(DcmDataSet *dataset, const char *keyword, int index, 
    const char **result)
{
    uint32_t tag = dcm_dict_tag_from_keyword(keyword);
    DcmElement *element = dcm_dataset_get(NULL, dataset, tag);
    return dcm_element_get_value_string(NULL, element, result, 0);
}

static struct dicom_file *
dicom_file_new(char *filename)
{
    struct dicom_file *d = g_slice_new0(struct dicom_file);
    d->filename = g_strdup(filename);

    d->filehandle = dcm_filehandle_create_from_file(NULL, d->filename);
    if (!d->filehandle) {
        dicom_file_destroy(d);
        return NULL;
    }

    d->file_metadata = dcm_filehandle_read_file_metadata(NULL, d->filehandle);
    if (!d->file_metadata) {
        dicom_file_destroy(d);
        return NULL;
    }
    ...
    const char *sop;
    get_tag_str(d->file_metadata, "MediaStorageSOPClassUID", 0, &sop);
...
static void level_destroy(struct level *l) {
  ...
  FREEF(dicom_file_destroy, l->dicom_file);
...
static bool is_type(struct dicom_file *d, struct allowed_types *types)
{
    ...
    for (int i = 0; i < types->n_types; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            const char *type;
            if (!get_tag_str(d->metadata, "ImageType", j, &type)) {
                level_destroy(l);
                return NULL;
            }
...
static struct level *
level_new(struct dicom_file *d)
{
    // try to read the rest of the dicom file as a pyr level
    d->metadata = dcm_filehandle_read_metadata(NULL, d->filehandle);
    if (!d->metadata) {
        return NULL;
    }

    d->bot = dcm_filehandle_read_bot(NULL, d->filehandle, d->metadata);
    if (!d->bot) {
        /* Try to build the BOT instead.
         */
        d->bot = dcm_filehandle_build_bot(NULL, d->filehandle, d->metadata);
    }
    if (!d->bot) {
        return NULL;
    }

    level *l = g_slice_new0(struct level);

    if (!get_tag_int(d->metadata, "TotalPixelMatrixColumns", &l->image_width) ||
        !get_tag_int(d->metadata, "TotalPixelMatrixRows", &l->image_height) ||
        !get_tag_int(d->metadata, "Columns", &l->tile_w) ||
        !get_tag_int(d->metadata, "Rows", &l->tile_h)) {
        level_destroy(l);
        return NULL;
    }

    // ImageType must be one of the combinations we accept
    if (!is_type(d, allowed_level_types)) {
        level_destroy(l);
        return NULL;
    }
...
static bool find_levels(gpointer key, gpointer value, gpointer user_data)
{
    const char *filename = (const char *) key;
    struct dicom_file *d = (struct dicom_file *) value;
    GHashTable *level_hash = (GHasTable *) user_data;

    struct level *l = level_new(d);
    if (l) {
        g_hash_table_insert(level_hash, filename, l);
        return true;
    }
    else {
        return false;
...
static bool remove_bad_level(gpointer key, gpointer value, gpointer user_data)
{
  const char *filename = (const char *) key;
  struct level *l = (struct level *) value;
  struct dicom_file *d = l->dicom_file;
  const char *slide_id = (const char *) user_data;

  const char *this_slide_id;

  return get_tag_str(d->metadata, "SeriesInstanceUID", 0, &this_slide_id) &&
         strcmp(slide_id, this_slide_id) != 0;
}

static bool remove_bad_dicom(gpointer key, gpointer value, gpointer user_data)
{
  const char *filename = (const char *) key;
  struct dicom_file *d = (struct dicom_file *) value;
  const char *slide_id = (const char *) user_data;
...
static bool add_level_array(gpointer key, gpointer value, gpointer user_data)
{
    const char *filename = (const char *) key;
    struct level *l = (struct level *) value;
...
static bool find_associated(gpointer key, gpointer value, gpointer user_data)
{
    const char *filename = (const char *) key;
    struct dicom_file *d = (struct dicom_file *) value;
    GHashTable *associated_hash = (GHasTable *) user_data;

    struct associated_image *a = associated_new(d);
    if (a) {
...
static bool dicom_open(openslide_t *osr, const char *filename,
  ...
  while (name = g_dir_read_name(dir)) {
      ...
      struct dicom_file *d = dicom_file_new(filename)
      if (!d) {
          g_free(filename);
          continue;
      }

      g_hash_table_insert(dicom_file_hash, filename, d);
  }
  ...
  const char *slide_id;
  if (!get_tag_str(largest->d->metadata, "SeriesInstanceUID", 0, &slide_id)) {
      return;
  }

  // throw away all files which don't have this slide_id
  g_hash_table_foreach_remove(level_hash, remove_bad_level, slide_id);
  g_hash_table_foreach_remove(dicom_file_hash, remove_bad_dicom, slide_id);

  // compute the downsample for each sublevel
  g_hash_table_foreach(level_hash, set_downsample, largest);
  ...
  g_autoptr(GPtrArray) level_array = 
      g_ptr_array_new_full(level_count, level_destroy);
  ...
  osr->levels = (struct _openslide_level **) 
      g_ptr_array_steal(level_array, &osr->level_count);
  osr->data = data;
  osr->ops = &dicom_ops;
